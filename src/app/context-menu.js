...
export function initContextMenu(h) {
  ...
  document.addEventListener('contextmenu', (e) => {
    ...
    const bedDomId = bedFromTarget(e.target);
    const items = [];
    const beds = typeof handlers.getBeds === 'function' ? (handlers.getBeds() || []) : [];
    ...
    if (habitId) {
      ...
      if (!habit?.paused && !habit?.archived && handlers.moveHabitToBed) {
        items.push({ type: 'separator' });
        for (const bed of beds) {
          const inBed = (habit.bedId || '') === bed.id;
          items.push({
            label: (t.contextMoveToBed || 'Move to: {name}').replace('{name}', bed.name),
            action: () => handlers.moveHabitToBed(habitId, bed.id),
            disabled: inBed
          });
        }
        if (habit.bedId) {
          items.push({
            label: t.contextRemoveFromBed || 'Remove from bed',
            action: () => handlers.moveHabitToBed(habitId, '')
          });
        }
        items.push({
          label: t.contextNewBed || 'New bed…',
          action: () => handlers.createBed?.(habitId)
        });
      }
    ...
    } else if (bedDomId && handlers.fromDomBedId) {
      const bedId = handlers.fromDomBedId(bedDomId);
      if (bedId) {
        items.push(
          { label: t.contextRenameBed || 'Rename bed…', action: () => handlers.renameBed?.(bedId) },
          { label: t.contextDeleteBed || 'Delete bed', danger: true, action: () => handlers.deleteBed?.(bedId) },
          { type: 'separator' }
        );
      }
      items.push({ label: t.contextNewBed || 'New bed…', action: () => handlers.createBed?.() });
